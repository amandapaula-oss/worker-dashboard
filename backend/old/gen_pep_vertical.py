...
def norm_vert(v):
    v = v.strip()
    if v == 'Finance & Insurance': return 'Finance'
...
final = {}
for v, pep in expected_raw:
    final[pep.strip()] = norm_vert(v)

with open('pep_vertical.csv', 'w', newline='', encoding='utf-8-sig') as f:
    w = csv.writer(f)
    w.writerow(['pep', 'vertical'])
    for pep, vert in sorted(final.items()):
        w.writerow([pep, vert])

print(f"pep_vertical.csv gerado: {len(final)} entradas")
cnt = Counter(final.values())
for v, n in sorted(cnt.items()):
    print(f"  {v:<15}: {n:>3} PEPs")
