...
@app.get("/api/pessoas")
def get_pessoas(user=Depends(get_current_user)):
    """Lista de pessoas agregadas da nova_base, organizadas por CPF.

    Para cada pessoa retorna:
    - cpf, nome, numero_pessoal
    - empresa (mais recente), funcao (mais recente), area, macro_area
    - tipo_contrato (CLT/PJ via Mapa Pessoas)
    - billable_category (custo/despesa/etc)
    - custo_mensal_medio, custo_total, meses_ativos
    - primeiro_periodo, ultimo_periodo
    """
    df = _get_nova_base().copy()
    if df.empty:
        return []

    df["_nome"] = df.get("nome_pessoa", pd.Series("", index=df.index)).astype(str).str.upper().str.strip()
    cpf_d = df.get("cpf", pd.Series("", index=df.index)).astype(str).str.replace(r"[^\d]", "", regex=True)
    df["_cpf"] = cpf_d.where(cpf_d.str.len() >= 11, "")

    # Enriquece CPF via Pessoal (de-para nome → cpf)
    map_nome_cpf, _ = _carregar_pessoal_depara()
    if map_nome_cpf:
        cpf_via_nome = df["_nome"].map(map_nome_cpf).fillna("")
        df["_cpf"] = df["_cpf"].where(df["_cpf"] != "", cpf_via_nome)

    # Filtra linhas com nome ou cpf
    df = df[(df["_nome"] != "") | (df["_cpf"] != "")]
    if df.empty:
        return []

    df["_periodo"] = df["periodo"].astype(str)
    df["_chave"] = df["_cpf"].where(df["_cpf"] != "", "nome:" + df["_nome"])

    # Custo mensal: usa custo_gerencial_sap (CLT/gerencial) ou valor_liquido (PJ)
    custo_ger = pd.to_numeric(df.get("custo_gerencial_sap", 0), errors="coerce").fillna(0)
    val_liq = pd.to_numeric(df.get("valor_liquido", 0), errors="coerce").fillna(0)
    df["_custo_mes"] = custo_ger
    is_pj_fonte = df["fonte"].astype(str) == "PJs"
    df.loc[is_pj_fonte, "_custo_mes"] = val_liq[is_pj_fonte]

    # Por (pessoa, periodo) — pega o custo mais alto entre as fontes (evita double-count)
    custo_mes = (df[df["_custo_mes"] != 0]
                 .groupby(["_chave", "_periodo"])["_custo_mes"].max()
                 .reset_index())

    # Agrega por pessoa
    pessoa_info = df.sort_values("_periodo", ascending=False).groupby("_chave").agg(
        nome           = ("nome_pessoa",        lambda s: s.dropna().astype(str).iloc[0] if len(s.dropna()) else ""),
        cpf            = ("_cpf",               lambda s: s.iloc[0] if len(s) else ""),
        numero_pessoal = ("numero_pessoal",     lambda s: s.dropna().astype(str).iloc[0] if len(s.dropna()) else ""),
        empresa        = ("empresa",            lambda s: s.dropna().astype(str).iloc[0] if len(s.dropna()) else ""),
        funcao         = ("funcao",             lambda s: s.dropna().astype(str).iloc[0] if len(s.dropna()) else ""),
        area           = ("area",               lambda s: s.dropna().astype(str).iloc[0] if len(s.dropna()) else ""),
        macro_area     = ("macro_area",         lambda s: s.dropna().astype(str).iloc[0] if len(s.dropna()) else ""),
        billable_category = ("billable_category", lambda s: s.dropna().astype(str).iloc[0] if len(s.dropna()) else ""),
        primeiro_periodo  = ("_periodo",          "min"),
        ultimo_periodo    = ("_periodo",          "max"),
    ).reset_index()

    # tipo_contrato: pega das fontes CLTs/PJs primeiro; senão infere do tipo_contrato existente
    fontes_pessoais = df[df["fonte"].isin(["CLTs", "PJs"])][["_chave", "fonte"]].drop_duplicates()
    fontes_pessoais["tipo_contrato"] = fontes_pessoais["fonte"].map({"CLTs": "CLT", "PJs": "PJ"})
    fp_map = fontes_pessoais.drop_duplicates("_chave").set_index("_chave")["tipo_contrato"].to_dict()
    pessoa_info["tipo_contrato"] = pessoa_info["_chave"].map(fp_map).fillna("")

    # Custo médio e total
    cmes_agg = custo_mes.groupby("_chave").agg(
        custo_mensal_medio = ("_custo_mes", "mean"),
        custo_total        = ("_custo_mes", "sum"),
        meses_ativos       = ("_periodo",   "nunique"),
    ).reset_index()
    pessoa_info = pessoa_info.merge(cmes_agg, on="_chave", how="left").fillna({
        "custo_mensal_medio": 0, "custo_total": 0, "meses_ativos": 0
    })

    pessoa_info = pessoa_info.drop(columns=["_chave"])
    pessoa_info = pessoa_info.sort_values("nome").reset_index(drop=True)
    return _sanitize(pessoa_info.to_dict(orient="records"))


# ── Dados / Relações ──────────────────────────────────────────────────────────

_relacoes_cache: dict = {"data": None}

def _carregar_relacoes() -> dict:
    """Carrega Relações.xlsx (de-para Centro de Custo→Macro Área e Profit Center→Name).
    Cacheia em memória.
    """
    if _relacoes_cache["data"] is not None:
        return _relacoes_cache["data"]
    path = os.path.join(_BASE_DIR, "Relações.xlsx")
    if not os.path.exists(path):
        _relacoes_cache["data"] = {"centro_custo": [], "profit_center": []}
        return _relacoes_cache["data"]
    df = pd.read_excel(path, sheet_name="De Para")
    cc = df[["Centro de Custo", "Macro Área"]].dropna(how="all")
    cc = cc.rename(columns={"Centro de Custo": "centro_custo", "Macro Área": "macro_area"})
    cc = cc.dropna(subset=["centro_custo"]).reset_index(drop=True)
    pc = df[["Profit Centers", "Name"]].dropna(how="all")
    pc = pc.rename(columns={"Profit Centers": "profit_center", "Name": "name"})
    pc = pc.dropna(subset=["profit_center"]).reset_index(drop=True)
    data = {
        "centro_custo": cc.to_dict(orient="records"),
        "profit_center": pc.to_dict(orient="records"),
    }
    _relacoes_cache["data"] = data
    print(f"[relacoes] {len(data['centro_custo'])} centro_custo, {len(data['profit_center'])} profit_center carregados")
    return data


@app.get("/api/relacoes")
def get_relacoes(user=Depends(get_current_user)):
    """Retorna mapeamentos de Centro de Custo → Macro Área e Profit Center → Name."""
    return _carregar_relacoes()
